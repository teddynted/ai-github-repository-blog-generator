...
| 🎬 **Social Video Rendering** | Opt-in AWS Step Functions stage (`GenerateSocialVideos`) that renders the YouTube/Shorts/TikTok scripts into **MP4s** on **ECS Fargate** (FFmpeg + **Amazon Polly**), stores them in S3, and publishes a manifest — three formats in parallel, each fault-tolerant. Every scene gets a background: the **architecture diagram** (where the scene or its visual direction calls for one), an **AI-generated scene image** (Replicate FLUX/SDXL, opt-in via `ENABLE_SCENE_IMAGES`, using the same prompt logic as `storyboard-scenes`), or a **deep-slate title slide** fallback. Renders are **idempotent** (an existing MP4 is skipped unless `FORCE_RENDER=true`) ([docs](./docs/social-video.md)) |
