...
When a run does fire, **n8n** orchestrates a pipeline on the instance where **OpenClaw** analyses the repository, **Repository Memory** provides continuity across runs, and a **local LLM (Ollama, Qwen by default)** extracts a structured **engineering analysis**. That analysis is the contract handed to the writing stage: by default the same local model writes the content (**zero paid inference APIs**), or — by setting `BEDROCK_MODEL_ID` — **Claude on Amazon Bedrock** (IAM-authenticated, no API key) writes it for higher-quality, narrative technical content. See [Content Pipeline Redesign](./docs/content-pipeline-redesign.md). The instance is started and stopped by the schedule, so compute cost is bounded by that window rather than by webhook volume.
