...
| 🎬 **Social Video Rendering** | Opt-in AWS Step Functions stage (`GenerateSocialVideos`) that renders the YouTube/Shorts/TikTok scripts into **MP4s** on **ECS Fargate** (FFmpeg + **Amazon Polly**), stores them in S3, and publishes a manifest for downstream publishing — three formats in parallel, each fault-tolerant; started by the worker after a successful release run ([docs](./docs/social-video.md)) |
