...
func (g *Generator) VoiceOver(ctx context.Context, sb storyboard.Storyboard) (VoiceOverScript, error) {
	...
	for i, sc := range sb.Scenes {
		...
		// Speak file paths naturally ("an /etc/x" -> "the file /etc/x"). This adds
		// a word, so it runs BEFORE the fit-trim below so the extra word is counted
		// against the slot rather than overflowing it.
		narration = speakFilePaths(narration)
		...
		if base := speakFilePaths(collapse(sc.Narration)); wordCount(narration) < wordCount(base) {
			narration = base
		}
