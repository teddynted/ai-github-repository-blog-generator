...
// latestPublisher is an optional Publisher that can write the repository's
// stable latest/ area. The S3 and filesystem publishers implement it; a
// publisher that does not simply skips latest promotion.
type latestPublisher interface {
	PublishLatest(ctx context.Context, repoFullName string, assets []generation.Content) error
}
...
func (p *Pipeline) Run(ctx context.Context, req rc.Request) (Result, error) {
	...
	// the artifacts, then promote this release into the repository's latest/
...
func (p *Pipeline) publishMetadata(ctx context.Context, rctx *rc.ReleaseContext, assets []generation.Content, results []generation.PutResult) (generation.Content, contentmeta.Metadata) {
	...
	if err != nil {
		...
		return generation.Content{}, meta
	}
...
// promoteLatest re-publishes an approved release's artifacts into the
// repository's stable latest/ area (plus a latest.json pointer), so downstream
// systems can consume the newest approved content without knowing the tag. It
// reuses the already-generated bytes, so no read/copy permission is needed, and
// is best-effort — a promotion failure never fails a run whose content published.
func (p *Pipeline) promoteLatest(ctx context.Context, rctx *rc.ReleaseContext, content []generation.Content, metaContent generation.Content, generationID string) {
	lp, ok := p.Publisher.(latestPublisher)
	if !ok {
		return // publisher does not support a latest area
	}
	batch := make([]generation.Content, 0, len(content)+2)
	filenames := make([]string, 0, len(content))
	for _, a := range content {
		batch = append(batch, a)
		filenames = append(filenames, a.Filename())
	}
	if metaContent.Kind != "" {
		batch = append(batch, metaContent)
	}
	pointer, perr := contentmeta.BuildLatest(rctx.Release.Tag, generationID, filenames, nil).Content()
	if perr == nil {
		batch = append(batch, pointer)
	}
	if err := lp.PublishLatest(ctx, rctx.Repository.FullName, batch); err != nil {
		p.log("latest promotion failed", slog.String("release", rctx.Release.Tag), slog.String("error", err.Error()))
		return
	}
	p.log("promoted release to latest",
		slog.String("release", rctx.Release.Tag),
		slog.String("generationId", generationID),
		slog.Int("artifacts", len(batch)))
}
