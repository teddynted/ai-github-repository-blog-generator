...
func (g *Generator) planPrompt(rctx *rc.ReleaseContext) string {
	...
	b.WriteString("- HEADINGS: the 4–7 specific, story-driven section headings in order, one per line under this field. Each must be concrete to this system, not a generic template heading. Every word in a heading must be a REAL, correctly-spelled English word or an actual product/technology/service name (e.g. Lambda, EventBridge, CloudWatch, Claude) — never an invented, garbled, abbreviated, or nonsense token (e.g. never write \"claw\" for \"Claude\"), and never leave a heading grammatically incomplete.\n")
