...
func (g *Generator) articlePrompt(rctx *rc.ReleaseContext, title, plan string) string {
	ground := safeTruncate(contextBlock(rctx), g.promptBudget())
	var b strings.Builder
	b.WriteString("You are a Staff Software Engineer / Senior AWS Engineer writing a publication-quality engineering blog post — the register of the AWS Builders' Library, Stripe Engineering, the Cloudflare Blog, and the Netflix and Uber engineering blogs. It is NOT marketing copy, NOT documentation, and NOT a changelog. Write in the third person about the system; teach the reader; do not praise the project.\n\n")
	...
	b.WriteString("Before returning, verify: third-person engineering voice; no invented personal stories or motivations; not a release announcement; every claim grounded in the Release Context; at most two topic-specific Mermaid diagrams; complete sections; valid UTF-8 with no mojibake; reads as though a Staff Engineer wrote it. If any check fails, rewrite the article before returning it.\n\n")
