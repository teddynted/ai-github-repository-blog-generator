...
func narrationPrompt(title, typ, nextTitle, prior, draft, subject string, isLast bool) string {
	...
	if isOpening {
		// Opening scene: orient the viewer FIRST on the capability at the video's
		// centre, then hook. A video needs a clear beginning — one sentence of
		// context before tension — but it speaks to WHAT THE SYSTEM DOES, never to a
		// "release"/version/repository (see the evergreen rule below).
		subjectClause := ""
		if s := strings.TrimSpace(subject); s != "" {
			subjectClause = fmt.Sprintf(" The video's subject is %q. In the FIRST sentence, orient the viewer — name the capability or idea at its centre and why it matters, phrased as what the system now does (e.g. \"Observability turns an invisible EC2 schedule into…\"), NOT as \"this release adds…\". Do not read the title verbatim.", s)
		}
	...
	if typ == "conclusion" || (isLast && !isOpening) {
		sceneHint = "This is the CLOSING scene: land the video in three quick beats grounded in what it covered — the problem this solved, the reusable engineering pattern it demonstrates, and what it now makes possible. Close on ONE short, forward-looking line about the capability itself (do NOT invite the viewer to \"explore the release\" or mention a release/version). Ground every claim in the DRAFT; do not introduce products, numbers, or platform claims the draft did not state.\n"
	}
	bridge := ""
	...
	return terminology.Base().PromptClause() + "\n\n" + fmt.Sprintf(
		"You are a senior AWS platform engineer narrating one scene of a technical explainer video for an "+
			...
			"Speak to the capability, not the shipment: NEVER say \"this release\", \"the release\", \"this update\", "+
			"\"this version\", or \"this feature\", NEVER state a version number, and NEVER name the repository. Describe "+
			"what the system now DOES — say \"CloudWatch logs every exit inline\", not \"this release adds CloudWatch "+
			"logging\"; say \"one feature ships with zero fixes\" only as \"the change needed no fixes\". The viewer should "+
			"learn the behaviour, not that a version shipped it.\n"+
			...
			"Speak configuration in prose, never as literal syntax: when the DRAFT shows an environment variable or "+
			"config assignment (for example LLM_PROVIDER=ollama), do NOT read the raw KEY=value or \"set KEY to value\" "+
			"form aloud. Say what it selects using the thing's proper name — \"Ollama is the in-house default\", "+
			"\"pointing the provider at Amazon Bedrock instead\" — and write every product name with its correct "+
			"capitalisation (Ollama, not ollama), even where the draft shows it lowercased inside a config token.\n"+
