...
// CloudWatch metric names emitted by the optimizer.
const (
	MetricOptimizationRuns       = "OptimizationRuns"
	MetricRecommendationCount    = "RecommendationCount"
	MetricPromptVersion          = "PromptVersion"
	MetricOptimizationConfidence = "OptimizationConfidence"
	MetricPerformanceImprovement = "PerformanceImprovement"
	MetricPredictionAccuracy     = "PredictionAccuracy"
	MetricTrendDetection         = "TrendDetectionCount"
	MetricOptimizationDuration   = "OptimizationDurationMs"
	MetricRecommendationAccept   = "RecommendationAcceptanceRate"
)

// LogMetricsPublisher writes metrics to the structured log, standing in for a
// real CloudWatch PutMetricData adapter. It has the exact shape a real adapter
// needs, so swapping in the AWS SDK requires no caller changes.
