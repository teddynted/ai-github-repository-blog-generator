...
func (g *Generator) prompt(pkg ReleasePackage) string {
	...
	b.WriteString("- If the evidence references a service ONLY as an out-of-band management/status channel (e.g. AWS Systems Manager used to poll build status), do NOT model it as a diagram component. Instead add a short `### <Service> Note` subsection under `### Cross-cutting` stating that it is referenced only as an out-of-band management/status channel and is intentionally omitted as a primary diagram component because it is not part of the runtime topology this release represents.\n")
