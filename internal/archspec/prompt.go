...
func (g *Generator) prompt(pkg ReleasePackage) string {
	...
	b.WriteString("List every component the evidence supports. Give each a stable, role-based canonical Name (e.g. \"Build Artifact Bucket\", \"AMI Builder Instance\", \"Compute Schedule\", \"Start/Stop Function\", \"Runtime Compute Host\") rather than the bare AWS service name, and use that exact Name for the component everywhere else in this document. If the release's workflow has distinct phases (for example an out-of-band build/provisioning path versus a scheduled runtime path), group the components under `### ` plane subsections named for those phases plus a `### Cross-cutting` subsection for shared IAM/observability; if there is only one phase, list them under a single `### ` group. For each component, provide: Name; Type (e.g. compute, storage, messaging, serverless, networking, IAM, external); AWS Service (or External System); Purpose (1–2 concise sentences on the component's architectural responsibility — do NOT restate the operational flow, security, or other sections, and do not pad with 'Exists so …' or 'This is the release's core change …' asides); Relationships (adjacent components, inputs, outputs, dependencies); Repository Evidence (the specific template/resource/file that proves it exists). Type a machine image (an AMI) as `image-artifact` with AWS Service `Amazon EC2 AMI` — not `storage` — so a renderer draws it as a distinct build output rather than a data store; reserve `storage` for actual data stores (S3, EFS, EBS volumes). ")
	b.WriteString("Emit EACH component in exactly this nested-bullet shape so a downstream parser can read it deterministically — the FIRST line is the bold canonical Name, its fields follow as indented sub-bullets:\n")
	b.WriteString("- **<canonical Name>**\n")
	b.WriteString("  - Type: <compute|storage|messaging|serverless|networking|IAM|external|image-artifact>\n")
	b.WriteString("  - AWS Service: <AWS service, or External System>\n")
	b.WriteString("  - Purpose: <1–2 sentences>\n")
	b.WriteString("  - Relationships: <adjacent components, inputs, outputs>\n")
	b.WriteString("  - Repository Evidence: <specific template/resource/file>\n\n")
	b.WriteString("## Connections\n")
	b.WriteString("Describe every connection between components. Source and Target MUST each be the exact canonical component Name defined in Components above — never a raw AWS service name, and never with a parenthetical qualifier appended (put any qualifier in Purpose instead). ")
	b.WriteString("When two components interact through genuinely distinct logical actions that differ in direction or mechanism — for example a one-way control call (`run-instances`), a bidirectional out-of-band status/polling channel, and a separate cleanup/teardown call — emit a SEPARATE connection for each distinct action. Otherwise collapse repeated interactions of the same kind into ONE connection and summarize them in Purpose.\n")
	b.WriteString("Emit EVERY connection in exactly this nested-bullet shape so a downstream parser can read it deterministically:\n")
	b.WriteString("- Source: **X** → Target: **Y**\n")
	b.WriteString("  - Protocol/mechanism: <e.g. event, HTTPS, IAM-scoped API call>\n")
	b.WriteString("  - Purpose: <one concise sentence>\n")
	b.WriteString("  - Direction: <Source → Target, written with the same arrow as the header>\n")
	b.WriteString("  - Repository Evidence: <specific evidence>\n")
	b.WriteString("Use the bidirectional arrow `⇄` (in both the header and Direction) ONLY for a polling/status channel; use `→` for every one-way connection.\n\n")
	...
	b.WriteString("Keep this a terse checklist, NOT prose — do not repeat Purpose, Operational Flow, Security, or per-connection detail already stated above. Cover only, in short bullets: plane layout (stacked horizontal planes with left→right flow inside each, when Components are grouped into planes); node ordering within each plane; the single emphasized cross-plane hand-off edge; primary vs supporting components (render IAM as a badge/annotation and observability as a supporting node attached to what it governs, not inline flow steps); prohibited elements (no VPC boundary, subnets/security groups, or general-purpose event bus unless the topic is explicitly about them); icon mapping (component → AWS icon); and a suggested canvas/viewBox aspect ratio.\n\n")

	b.WriteString("CRITICAL RULES:\n")
	b.WriteString("- Show the release's change IN CONTEXT, not in isolation. Include the components its own workflow exercises end to end — the trigger that starts it, the compute that executes the change, and the storage/queue/target it reads or writes — even when some of that infrastructure pre-existed, scoping each to THIS release's use of it. A specification with a single component, or with no connections between components, is INVALID: one node is not an architecture. The exclusions below remove UNRELATED platform services (ingestion, a general-purpose event bus, inference/runtime services), never the release's own execution path.\n")
	b.WriteString("- Whenever two or more components exist (they almost always do), you MUST populate the Connections section with every interaction between them; never leave it empty or omit it.\n")
	...
	b.WriteString("- If the evidence references a service ONLY as an out-of-band management/status channel (e.g. AWS Systems Manager used to poll build status and coordinate remote cleanup), do NOT model it as a diagram component. Instead add a `### <Service> Note` subsection under `### Cross-cutting` of AT MOST two sentences: the first naming its concrete role(s) in this release's workflow, the second stating it is intentionally omitted from Components because it is not part of the runtime topology this release represents. Do not narrate the channel's internal exchange.\n")
...
var baselineComponentTerms = []string{"claw", "openclaw", "bedrock", "anthropic", "n8n", "kafka"}
