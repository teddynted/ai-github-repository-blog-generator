// Command tiktok adapts a GitHub Release's generated artifacts — Release Context
// (M2), Technical Blog (M3), Storyboard (M4), Voice-over (M5), long-form YouTube
// Script (M6), and YouTube Shorts (M7) — into a collection of TikTok-native
// educational videos (Milestone 8).
//
// It reads a Release Context JSON and reuses any artifacts you pass (via --blog,
// --storyboard, --voiceover, --youtube, --shorts), generating the rest, then
// emits the TikToks as Markdown (default) or JSON.
//
// Usage:
//
...
//
//	# Generate the whole chain via the Anthropic API, then the TikToks as JSON:
//	go run ./cmd/tiktok --context ctx.json --format json --out tiktok.json
package main
...
func run(args []string) int {
	fs := flag.NewFlagSet("tiktok", flag.ContinueOnError)
	...
	maxVideos := fs.Int("max", 6, "maximum number of TikToks to generate")
	...
	pkg, err := buildPackage(ctx, rctx, *blogPath, *sbPath, *voPath, *ytPath, *shPath, mdl)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 1
	}

	col, err := (&tiktok.Generator{Model: mdl, MaxVideos: *maxVideos}).TikTok(ctx, pkg)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 1
	}
	...
	if *outPath != "" {
		fmt.Fprintf(os.Stderr, "wrote %s (%d videos)\n", *outPath, col.Metadata.VideoCount)
	}
	return 0
}
...
// buildPackage assembles the ReleasePackage, reusing any artifacts provided and
// generating the rest from the Release Context.
func buildPackage(ctx context.Context, rctx *rc.ReleaseContext, blogPath, sbPath, voPath, ytPath, shPath string, mdl releasegen.Model) (tiktok.ReleasePackage, error) {
	post, err := resolveBlog(ctx, blogPath, rctx, mdl)
	if err != nil {
		return tiktok.ReleasePackage{}, err
	}
	...
	yt, err := resolveYouTube(ctx, ytPath, rctx, post, sb, vo, mdl)
	if err != nil {
		return tiktok.ReleasePackage{}, err
	}
	sh, err := resolveShorts(ctx, shPath, rctx, post, sb, vo, yt, mdl)
	if err != nil {
		return tiktok.ReleasePackage{}, err
	}
	return tiktok.ReleasePackage{Context: rctx, Blog: post, Storyboard: sb, VoiceOver: vo, YouTube: yt, Shorts: sh}, nil
}
