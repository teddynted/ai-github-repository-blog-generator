// Command visualassets converts a GitHub Release's generated artifacts —
// Release Context (M2), Technical Blog (M3), Storyboard (M4), Voice-over (M5),
// YouTube Script (M6), YouTube Shorts (M7), and TikTok (M8) — into structured,
// provider-neutral AI IMAGE PROMPTS for thumbnails, social graphics, blog
// headers, banners, and technical illustrations (Milestone 9).
//
// It reads a Release Context JSON and reuses any artifacts you pass, generating
// the rest, then emits the prompts as Markdown (default) or JSON. It produces
// prompts, not images.
//
// Usage:
//
...
//
//	# Generate the whole chain via the Anthropic API, then the prompts as JSON:
//	go run ./cmd/visualassets --context ctx.json --format json --out assets.json
package main
...
func run(args []string) int {
	fs := flag.NewFlagSet("visualassets", flag.ContinueOnError)
	...
	maxAssets := fs.Int("max", 14, "maximum number of assets to generate")
	format := fs.String("format", "md", "output format: md | json")
	model := fs.String("model", "", "model id (Anthropic; provider default when empty)")
	offline := fs.Bool("offline", false, "do not call the model (deterministic prompts)")
	...
	col, err := (&visualassets.Generator{Model: mdl, MaxAssets: *maxAssets}).VisualAssets(ctx, pkg)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 1
	}
	...
	if *outPath != "" {
		fmt.Fprintf(os.Stderr, "wrote %s (%d assets)\n", *outPath, col.Metadata.AssetCount)
	}
	return 0
}
...
func buildPackage(ctx context.Context, rctx *rc.ReleaseContext, src sources, mdl releasegen.Model) (visualassets.ReleasePackage, error) {
	post, err := resolveBlog(ctx, src.blog, rctx, mdl)
	if err != nil {
		return visualassets.ReleasePackage{}, err
	}
	...
	return visualassets.ReleasePackage{Context: rctx, Blog: post, Storyboard: sb, VoiceOver: vo, YouTube: yt, Shorts: sh, TikTok: tt}, nil
}
