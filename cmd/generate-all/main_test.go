...
func TestRunGeneratesAllArtifactsOffline(t *testing.T) {
	...
	if len(m.Stages) != 14 {
		t.Errorf("want 14 stages, got %d", len(m.Stages))
	}
	if m.Produced != 14 || m.Failed != 0 {
		t.Errorf("want produced=14 failed=0, got produced=%d skipped=%d failed=%d", m.Produced, m.Skipped, m.Failed)
	}
