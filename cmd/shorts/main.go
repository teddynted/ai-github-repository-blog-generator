// Command shorts mines a GitHub Release's generated artifacts — Release Context
// (M2), Technical Blog (M3), Storyboard (M4), Voice-over (M5), and long-form
// YouTube Script (M6) — for the best technical moments and plans a collection of
// standalone 30–60 second YouTube Shorts (Milestone 7).
//
// It reads a Release Context JSON and reuses any artifacts you pass (via --blog,
// --storyboard, --voiceover, --youtube), generating the rest, then emits the
// Shorts as Markdown (default) or JSON.
//
// Usage:
//
...
func run(args []string) int {
	...
	pkg, err := buildPackage(ctx, rctx, *blogPath, *sbPath, *voPath, *ytPath, mdl)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 1
	}

	col, err := (&shorts.Generator{Model: mdl, MaxShorts: *maxShorts}).YouTubeShorts(ctx, pkg)
	if err != nil {
		fmt.Fprintf(os.Stderr, "error: %v\n", err)
		return 1
	}
	...
	if *outPath != "" {
		fmt.Fprintf(os.Stderr, "wrote %s (%d shorts)\n", *outPath, col.Metadata.ShortCount)
	}
	return 0
}
...
func buildPackage(ctx context.Context, rctx *rc.ReleaseContext, blogPath, sbPath, voPath, ytPath string, mdl releasegen.Model) (shorts.ReleasePackage, error) {
	post, err := resolveBlog(ctx, blogPath, rctx, mdl)
	if err != nil {
		return shorts.ReleasePackage{}, err
	}
	...
	return shorts.ReleasePackage{Context: rctx, Blog: post, Storyboard: sb, VoiceOver: vo, YouTube: yt}, nil
}
